...
class LendClient(WorksheetClient):
    ...
    def lend_book(self, lend_model: LendModel):
        ...
        book_df.loc[book_df["ID"] == lend_model.book_id, "USED"] += (
            1  # = self.sheets[SheetName.BOOK.value].loc[filter, "USED"] +
        )
        ...
        if lend_df.empty:
            ...
            self.sheets[SheetName.LEND.value] = pd.concat(
                [self.sheets[SheetName.LEND.value], serialized_book], ignore_index=True
            )
