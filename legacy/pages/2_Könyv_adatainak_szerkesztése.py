...
if len(st.session_state["book_sheet"]):
    gb = GridOptionsBuilder.from_dataframe(
        st.session_state["book_sheet"][
            ["ID", "AUTHOR", "TITLE", "ISBN", "QUANTITY", "USED"]
        ]
    )
    # configure selection
    ...
    if modal.is_open():
        with modal.container():
            if st.button("Igen"):
                ...
                print("ID", id.array[0])
                index = st.session_state["book_sheet"][
                    st.session_state["book_sheet"]["ID"] == id.array[0]
                ].index
                print("DEBUG:", id, index)
                st.session_state["book_sheet"] = st.session_state["book_sheet"].drop(
                    index
                )
                update_sheet(
                    st.session_state["workbook"], st.session_state["book_sheet"]
                )

                st.session_state["lend_sheet"] = check_if_book_at_disposal(
                    st.session_state["book_sheet"],
                    st.session_state["lend_sheet"],
                    st.session_state["lend_workbook"],
                )
        ...
        with st.sidebar:
            if len(selected_rows) != 0:
                ...
                quantity = st.number_input(
                    label="Quantity:", value=QUANTITY, min_value=min(QUANTITY, USED)
                )
            ...
            if submitted:
                ...
                if len(selected_rows) != 0:
                    ...
                    try:
                        ...
                        # update person data
                        used = st.session_state["book_sheet"].loc[
                            st.session_state["book_sheet"]["ID"] == BOOK_ID, "USED"
                        ]
                        st.session_state["book_sheet"].loc[
                            st.session_state["book_sheet"]["ID"] == BOOK_ID
                        ] = pd.DataFrame(
                            {
                                "ID": BOOK_ID,
                                "AUTHOR": author,
